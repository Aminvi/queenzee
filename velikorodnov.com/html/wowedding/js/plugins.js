;(function($){
	...
	$.mad_core = {
		...
		DOMLoaded: function(options) {
			...
			// responsive menu
			if($('#main-navigation').length) base.navInit.init(this);
		...
		syncOwlCarousel: {
			...
			bindEvents: function(){
				...
				this.collection.each(function(i, el){
					...
					$this.on('click', '.avatar', function(e){
						e.preventDefault();
						$this.trigger('next.owl.carousel');
						sync.trigger('next.owl.carousel');
					});
		...
		**/
		...
		navInit : {
			...
			touchNavEvent: function (base) {
				...
				$("li > a.hassub, li > .mega-title").on('click', function (e) {
					e.preventDefault();
				});
			...
			createResponsiveButtons : function () {
				...
				this.navHide = $('<a></a>', {
					...
					'class': 'responsive-nav-button',
					'href' : '#'
				}).insertBefore(this.navButton); 

			},

			menuNavToggle : function () {

				if ($(window).width() < 992 || $('.side-menu').length) {

					$('#menu > li').has('div').children('a').addClass('hassub');
				
					$('#menu > li').removeClass('current');

					$('#menu li a, #menu > li .mega-title').on('click', function (e) {

						$(this).parent().toggleClass('current');

			    		var target = $(this).parent().children(".sub-menu-wrap, ul");
				    	$(target).slideToggle();
					...
					var mainMenu = $("#menu");
					var mainMenuBtn = $("#responsive-nav-button");

				 	$win.on("click.Bst", function(event){		
						if ( 
			            mainMenu.has(event.target).length === 0 //checks if descendants of $box was clicked
			            &&
			            !mainMenu.is(event.target) //checks if the $box itself was clicked
			            &&
			            !mainMenuBtn.is(event.target) //checks if the $box itself was clicked
				        ){
							base.wrapper.removeClass('active');
						}
					});
